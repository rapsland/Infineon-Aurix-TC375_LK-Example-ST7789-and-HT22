...
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
void erasePFLASH(uint32_t sectorAddr);
void writePFLASH(uint32_t startingAddr);
...
uint8_t flashingVerifyPageErased(uint32_t address);

typedef struct
{
    void (*eraseSectors)(uint32_t sectorAddr, uint32_t numSector);
    uint8_t (*waitUnbusy)(uint32_t flash, IfxFlash_FlashType flashType);
    uint8_t (*enterPageMode)(uint32_t pageAddr);
    void (*load2X32bits)(uint32_t pageAddr, uint32_t wordL, uint32_t wordU);
    void (*writePage)(uint32_t pageAddr);
    void (*eraseFlash)(uint32_t sectorAddr);
    void (*writeFlash)(uint32_t startingAddr);
} Function;
...
void writePFLASH(uint32_t startingAddr)
{
    uint32_t page;                                                /* Variable to cycle over all the pages             */
    uint32_t offset;                                              /* Variable to cycle over all the words in a page   */
    ...
    {
        uint32_t pageAddr = startingAddr + (page * PFLASH_PAGE_LENGTH);   /* Get the address of the page              */
...
void verifyProgramFlash()
{
    uint32_t page;                                                /* Variable to cycle over all the pages             */
    uint32_t offset;                                              /* Variable to cycle over all the words in a page   */
    uint32_t errors = 0;                                          /* Variable to keep record of the errors            */

    /* Verify the written data */
    for(page = 0; page < PFLASH_NUM_PAGE_TO_FLASH; page++)                          /* Loop over all the pages      */
    {
        uint32_t pageAddr = PFLASH_STARTING_ADDRESS + (page * PFLASH_PAGE_LENGTH);    /* Get the address of the page  */
...
void writeDataFlash()
{
    ...
    {
        uint32_t pageAddr = DFLASH_STARTING_ADDRESS + (page * DFLASH_PAGE_LENGTH); /* Get the address of the page     */
...
void writeSensorDataToFlash(const SensorData* data) {
    ...
    IfxFlash_eraseMultipleSectors(SENSOR_DATA_FLASH_ADDRESS, (SENSOR_DATA_SIZE + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE);
    IfxFlash_waitUnbusy(FLASH_MODULE, PROGRAM_FLASH_0);

    // Write the sensor data to flash in 32-byte pages
    const uint8_t* dataPtr = (const uint8_t*)data;
    for (uint32_t offset = 0; offset < SENSOR_DATA_SIZE; offset += FLASH_PAGE_SIZE) {
        IfxFlash_enterPageMode(SENSOR_DATA_FLASH_ADDRESS + offset);
        IfxFlash_waitUnbusy(FLASH_MODULE, PROGRAM_FLASH_0);
        IfxFlash_loadPage2X32(SENSOR_DATA_FLASH_ADDRESS + offset, *((uint32_t*)(dataPtr + offset)), *((uint32_t*)(dataPtr + offset + 4)));
        IfxFlash_writePage(SENSOR_DATA_FLASH_ADDRESS + offset);
        IfxFlash_waitUnbusy(FLASH_MODULE, PROGRAM_FLASH_0);
    }
...
void readSensorDataFromFlash(SensorData* data) {
    if (flashingVerifyPageErased(SENSOR_DATA_FLASH_ADDRESS)) {
        // Initialize data with default values if the page is erased
        memset(data, 0, sizeof(SensorData));
        data->marker = SENSOR_DATA_MARKER;
    } else {
        // Read the sensor data from flash in 32-byte pages
        uint8_t* dataPtr = (uint8_t*)data;
        for (uint32_t offset = 0; offset < SENSOR_DATA_SIZE; offset += FLASH_PAGE_SIZE) {
            memcpy(dataPtr + offset, (const void*)(SENSOR_DATA_FLASH_ADDRESS + offset), FLASH_PAGE_SIZE);
        }

        // Verify the marker
        if (data->marker != SENSOR_DATA_MARKER) {
            // Initialize data with default values if the marker is not present
            memset(data, 0, sizeof(SensorData));
            data->marker = SENSOR_DATA_MARKER;
        }
    }
...
void wait1ms(void) {
    uint64_t startTicks = IfxStm_get(&MODULE_STM0);
    uint64_t waitTicks = IfxStm_getTicksFromMilliseconds(&MODULE_STM0, 10000);
    while ((IfxStm_get(&MODULE_STM0) - startTicks) < waitTicks) {
        // Wait for 1 millisecond
    }
}

uint8_t flashingVerifyPageErased(uint32_t address) {
    IfxFlash_verifyErasedPage(address);
    ...
    if (!IfxFlash_isErrorDetected(IfxFlash_Error_eraseVerify)) {
        ...
        return 0; // Flash Page not empty, is readable!!
    }
    IfxFlash_clearStatus(0);
    return 1; // Flash Page is empty, is not readable!!
}
